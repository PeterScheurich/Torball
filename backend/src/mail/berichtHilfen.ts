...
/** Ob eine Mail die Aufbewahrungsfrist (aufbewahrungTage Tage) ueberschritten hat: nur erledigt/
 *  ignoriert veralten ueberhaupt (ein "offener" Eintrag bleibt unbegrenzt liegen), gemessen ab dem
 *  letzten aktualisiertAm - das faellt fuer eine bereits klassifizierte Mail mit dem Zeitpunkt
 *  zusammen, an dem der Status gesetzt wurde (siehe PUT /mail-postfach/nachrichten/:id). Die
 *  Ursprungsmail bleibt im echten Postfach erhalten (siehe MailNachricht-Dokumentation), hier
 *  geht nur die lokale Kopie/Klassifikation verloren.
 *  "kanban" ist bewusst ausgenommen (Nutzer-Vorgabe) - die Mail ist die Quelle/Referenz einer
 *  Kanban-Karte (MailNachricht.kanbanKartenId <-> KanbanKarte.quellMailId) und soll nicht
 *  verschwinden, waehrend die Karte noch offen/in Arbeit ist. */
export function istVeraltet(mail: MailNachricht, jetzt: Date, aufbewahrungTage: number): boolean {
  if (!mail.manuellerStatus || mail.manuellerStatus === "kanban") return false;
