...
- Turniermodus, Spielzeit je Halbzeit (Standard 5 Min.), Anzahl Halbzeiten (Standard 2), Pause zwischen Halbzeiten (Standard 2 Min.), Pause zwischen Spielen (Standard 10 Min. – siehe Abschnitt 8, Zeitplanung), Seitenwechsel ja/nein
...
**Zeitplanung:** Alle Zeiten basieren auf der Turnier-Startzeit. Die geplante Startzeit jedes Spiels ergibt sich aus Spielzeit × Anzahl Halbzeiten + Pause zwischen Halbzeiten + **Pause zwischen Spielen** (konfigurierbarer Puffer zwischen zwei aufeinanderfolgenden Spielen auf demselben Feld, Standardwert 10 Minuten) – dieser zusätzliche Puffer ist nötig, weil im Torball nur die Netto-Spielzeit gezählt wird (siehe 6.1) und ein Spiel dadurch in der Praxis üblicherweise länger dauert als die angegebene Spielzeit. Es gibt eine geplante und eine voraussichtliche (aktuelle) Zeit; Mannschaften sehen immer die voraussichtliche Startzeit ihres nächsten Spiels.
